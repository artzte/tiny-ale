...
RSpec.describe 'Learning plan goals API', type: :request do
  ...
  describe 'GET /api/learning-plan-goals' do
    it 'returns a 200 with the expected count of learning plan goals in the expected order' do
      get '/api/learning-plan-goals', headers: json_request_headers
      ...
      expect(json['meta']['count']).to eq(LearningPlanGoal.where(active: true).count)
